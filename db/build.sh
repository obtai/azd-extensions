...
# The -X target is a PACKAGE path — the module path, not EXTENSION_ID. A -X
# against a package that does not exist is ignored without complaint, so read
# the real module path rather than assuming the two are the same string.
