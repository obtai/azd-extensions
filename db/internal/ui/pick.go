...
// Confirm asks a yes/no question, answering no wherever nothing is drawn: the
// one write this extension makes never happens without someone saying yes.
func Confirm(ctx context.Context, u UI, title string) (bool, error) {
	if !u.Interactive {
		return false, nil
	}
	var yes bool
	err := huh.NewForm(huh.NewGroup(huh.NewConfirm().Title(title).Value(&yes))).
		WithTheme(huh.ThemeCharm()).
		WithOutput(os.Stderr).
		WithShowHelp(false).
		RunWithContext(ctx)
...
// Note is an aside on stderr: something worth knowing that is not the answer.
// Lines are styled one at a time; lipgloss pads a multi-line block out to its
// widest line, which leaves trailing whitespace down the right-hand side.
